...
'''
AL
en2de/data/en2de.dev.en.bpe.txt -> en2de/data/en2de.dev.de.bpe.txt
27.616716616716616

en2de/data/en2de.test.en.bpe.txt -> en2de/data/en2de.test.de.bpe.txt
26.565237436606733
...
def get_data(type_ref, type_dataset):
    ...
    file_bleu_trans = 'trans/BLEU_trans_en2de.txt'   # bleu dev: line 4-13, test line 16-25
    #lineNum = 28 if type_dataset=='dev' else 32 
    pattern = 'BLEU = ([^,]+),'
    #line = open('BLEU.md','r').readlines()[lineNum]
    #round(float(re.findall(pattern, line)[0]), 2)

    #from rnn/BLEU_rnn_en2de.txt line 28(greedy),29(beamsearch) for dev, and line 32(greedy), 33(beamsearch) for test 
    trans_bleu_base1, trans_bleu_base2 = (25.46, 24.23) if type_dataset=='dev' else (27.44,26.63)  # beam search with size 11
    rnn_bleu_base1, rnn_bleu_base2 = (None, None) if type_dataset=='dev' else (20.82, 19.76) # 23.94 #?
    marker_size = 120
    baseline_AL,base_AL_l, base_AL_r = (27.6167166, 26.7, 28) if type_dataset == 'dev' else (26.565237, 25.7, 27)

    lineNum = 1 # 1 if mean, 3 if corpus ave 
    trans_AP = list(map(float, open('trans/en2de_trans_{}_AP.txt'.format(type_dataset), 'r').readlines()[lineNum].split()))[:-1]  # remove the last: wait10
    rnn_AP   = list(map(float, open('rnn/en2de_rnn_{}_AP.txt'.format(type_dataset), 'r').readlines()[lineNum].split()))
    trans_AL = list(map(float, open('trans/en2de_trans_{}_AL.txt'.format(type_dataset), 'r').readlines()[lineNum].split()))[:-1]  # remove the last: wait10
    ...
    fig.savefig('en2de_bleu_AP_{}.pdf'.format(type_dataset))
    '''
    fig = plt.figure()
    ...
    plt.tight_layout()  # make room for the xlabel
    fig0.savefig('en2de_bleu_AL_{}.pdf'.format(type_dataset))
    ...
    plt.tight_layout()  # make room for the xlabel
    fig2.savefig('en2de_bleu_waitk_{}.pdf'.format(type_dataset))
