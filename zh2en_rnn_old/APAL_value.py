...
tgt=./catchup.w<i>.unbpe.txt, from /mnt/scratch/kaibo/proj/fordecoding_catchup/decode_result.w<i>.unbpe.txt (decoded sentences 616 lines)
tgt=./waitk.w<i>.unbpe.txt, from /mnt/scratch/kaibo/proj/fordecoding/decode_result.w<i>.unbpe.txt (decoded sentences 616 lines)
zh2en
src=/zh2enout_rnn/data/zh2en.dev.en.bpe.txt
tst=/zh2enout_rnn/dev/w<i>.bpe
'''
import sys
import os
...
def APAL_from_file(file_src, dir_tgt, type_dataset, type_bpe, model='trans', is_clean=False, is_weight_ave=False, metric='AP'):
    ...
    title_base = 'zh2en_{}_scatter_{}_{}'.format(model, type_dataset, metric)
    k_max = 10#9 if model=='rnn' else 10
    ...
    for waitk in range(1, k_max+1):
        de = open('{}/w{}.pred'.format(dir_tgt,waitk), 'r').readlines()
        ...
        for i, (x, y) in enumerate(zip(len_x, len_y)):
            ...
            if x < 1 or y < 1: print('{}/w{}.pred'.format(dir_tgt, waitk), waitk, x, y, i+1)
...
def plot_all(src, tgt_dir, type_dataset, model="trans", type_bpe=True, metric='AP'):
    ...
    fig.savefig('{}/zh2en_{}_{}_{}.pdf'.format(model, model, type_dataset, metric))
    f = open('{}/zh2en_{}_{}_{}.txt'.format(model, model, type_dataset, metric), 'w')
    print('{}/zh2en_{}_{}_{}.txt'.format(model, model, type_dataset, metric))
...
if __name__=="__main__":
    if len(sys.argv) < 2:
        ...
        src = '../testdata/{}_src.bpe'.format(type_dataset)
